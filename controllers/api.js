...
exports.getFoursquare = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'foursquare' });
  async.parallel({
...
exports.getTumblr = (req, res, next) => {
  ...
  const { accessToken, tokenSecret } = _.find(req.user.tokens, { kind: 'tumblr' });
...
exports.getFacebook = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'facebook' });
...
exports.getGithub = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'github' });
...
exports.getTwitter = (req, res, next) => {
  ...
  const { accessToken, tokenSecret } = _.find(req.user.tokens, { kind: 'twitter' });
  var T = new Twit({
...
exports.getVenmo = (req, res, next) => {
  ...
  var { accessToken } = _.find(req.user.tokens, { kind: 'venmo' });
...
exports.postVenmo = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'venmo' });
  var formData = {
    access_token: accessToken,
    note: req.body.note,
...
exports.getLinkedin = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'linkedin' });
...
exports.getInstagram = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'instagram' });
...
exports.getPayPal = (req, res, next) => {
  ...
  paypal.payment.create(paymentDetails, (err, payment) => {
    ...
    const { links } = payment;
    const approvalLink = _.find(links, (link) => {
...
exports.getPinterest = (req, res, next) => {
  ...
  const { accessToken } = _.find(req.user.tokens, { kind: 'pinterest' });
  request.get({ url: 'https://api.pinterest.com/v1/me/boards/', qs: { access_token: accessToken }, json: true }, (err, request, body) => {
    if (err) {
      return next(err);
    }
